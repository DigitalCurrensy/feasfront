...
A missing measurement, or a value that is not a finite number, fails the site as `missing` and does not add the other gates. Otherwise every limit the numbers miss is reported, in order: `slope`, `sun`, `earth`, `night`. The site is ok only when that list is empty.
